...
- Name: auth/login.spec.ts >> Login >> deve autenticar usuário cadastrado com sucesso
...
Locator: getByRole('link', { name: /logout|sair/i })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('link', { name: /logout|sair/i })
...
```ts
  ...
  10 |     this.username = page.locator('#username');
  11 |     this.password = page.locator('#password');
  12 |     this.loginButton = page.getByRole('button', {
  13 |       name: /login|entrar/i,
  14 |     });
  15 |
  16 |     this.logoutLink = page.getByRole('link', {
  17 |       name: /logout|sair/i,
  18 |     });
  19 |   }
  20 |
  ...
  25 |   }
  26 |
  27 |   async expectLoggedIn(): Promise<void> {
> 28 |     await expect(this.logoutLink).toBeVisible();
     |                                   ^ Error: expect(locator).toBeVisible() failed
  29 |   }
  30 | }
```
