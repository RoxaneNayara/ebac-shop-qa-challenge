...
Locator: locator('.pull-right.top-cart.hidden-xs').locator('.woocommerce-Price-amount.amount')
Expected substring: "0,00"
Error: strict mode violation: locator('.pull-right.top-cart.hidden-xs').locator('.woocommerce-Price-amount.amount') resolved to 2 elements:
    1) <span class="woocommerce-Price-amount amount">…</span> aka getByRole('button', { name: ' Cart : R$0,00' })
    2) <span class="woocommerce-Price-amount amount">…</span> aka locator('span').filter({ hasText: 'R$' }).nth(3)

Call log:
  - Expect "toContainText" with timeout 5000ms
  - waiting for locator('.pull-right.top-cart.hidden-xs').locator('.woocommerce-Price-amount.amount')
...
```ts
  ...
  9  |   private readonly cart: Locator;
  10 |   private readonly cartAmount: Locator;
  11 |   private readonly cartItems: Locator;
  12 |
  13 |   constructor(private readonly page: Page) {
  14 |     this.orderReceivedTitle = page.getByRole('heading', {
  15 |       name: 'Pedido recebido',
  16 |     });
  17 |
  18 |     this.orderNumber = page.locator(
  19 |       '.woocommerce-order-overview__order strong'
  20 |     );
  21 |
  22 |     this.orderTotal = page.locator(
  23 |       '.woocommerce-order-overview__total strong'
  24 |     );
  25 |
  26 |     this.paymentMethod = page.locator(
  27 |       '.woocommerce-order-overview__payment-method strong'
  28 |     );
  29 |     this.cartSummary = page.getByText(/Cart:\s*R\$0,00/i);
  30 |
  31 |     this.cart = page.locator('.pull-right.top-cart.hidden-xs');
  32 |
  33 |     this.cartAmount = this.cart.locator(
  34 |       '.woocommerce-Price-amount.amount'
  35 |     );
  36 |
  37 |     this.cartItems = this.cart.locator(
  38 |       '.mini-cart-items'
  39 |     );
  40 |   }
  41 |
  42 |   async expectOrderReceived(): Promise<void> {
  43 |     await expect(this.orderReceivedTitle).toBeVisible();
  44 |   }
  45 |
  46 |   async expectOrderNumber(): Promise<void> {
  47 |     await expect(this.orderNumber).toBeVisible();
  48 |     await expect(this.orderNumber).not.toBeEmpty();
  49 |   }
  50 |
  51 |   async expectOrderTotal(expectedTotal: string): Promise<void> {
  52 |     await expect(this.orderTotal).toContainText(expectedTotal);
  53 |   }
  54 |
  55 |   async expectPaymentMethod(expectedMethod: string): Promise<void> {
  56 |     await expect(this.paymentMethod).toContainText(expectedMethod);
  57 |   }
  58 |
  59 |   async expectProduct(
  60 |     productName: string,
  61 |     quantity: number
  62 |   ): Promise<void> {
  63 |     const row = this.page
  64 |       .locator('.woocommerce-table__line-item')
  65 |       .filter({ hasText: productName });
  66 |
  67 |     await expect(row).toBeVisible();
  68 |     await expect(row).toContainText(productName);
  69 |     await expect(row).toContainText(String(quantity));
  70 |   }
  71 |
  72 |   async expectCustomerContact(
  73 |     expectedPhone: string,
  74 |     expectedEmail: string
  75 |   ): Promise<void> {
  76 |     const normalizedPhone = expectedPhone.replace(/\D/g, '');
  77 |
  78 |     const pageText = await this.page.locator('body').innerText();
  79 |     const normalizedPageText = pageText.replace(/\D/g, '');
  80 |
  81 |     expect(normalizedPageText).toContain(normalizedPhone);
  82 |
  83 |     await expect(
  84 |       this.page.getByText(expectedEmail, { exact: true })
  85 |     ).toBeVisible();
  86 |   }
  87 |   async expectCartEmpty(): Promise<void> {
  88 |     await expect(this.cart).toBeVisible();
  89 |     await expect(this.cartItems).toHaveText('0');
> 90 |     await expect(this.cartAmount).toContainText('0,00');
     |                                   ^ Error: expect(locator).toContainText(expected) failed
  91 |   }
  92 | }
```
